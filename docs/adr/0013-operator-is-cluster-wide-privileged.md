...
been through the pipeline): `make test-e2e` on both Valkey lines, 51/51 green each, including
...
this one; data and Sentinel pods with `runAsUser`, `runAsGroup` and `fsGroup` 999 on top,
...
no CRD field, no `baseline` level and no opt-out. The stated decision was the wrong one: the
...
`runAsNonRoot: true` and `seccompProfile: RuntimeDefault` — plus `runAsUser`, `runAsGroup` and
`fsGroup` 999 on data and Sentinel pods — and `allowPrivilegeEscalation: false`,
`readOnlyRootFilesystem: true` and `capabilities.drop: [ALL]` on every container and init
container, the sidecar and the third-party exporter included. The one root container the
...
operator built exists, kept until every ordinal holds a rootless pod proven ours) and kept out
of the pod-spec hash, so a pod created from that template keeps it in its spec after the
template drops it (ADR 0032 D2, D4); a template carrying it passes `baseline`, not
...
* ~~**A `restricted`-PSA namespace will reject the generated pods outright**, so the operator
  ...
  template drops it, and is reported until it is replaced for another reason. The operator
...
* ~~**Workload pods have no `securityContext` (open)** while the operator's own Deployment sets
  ...
  **(Closed 2026-09-26 by [ADR 0032](0032-generated-pods-run-rootless.md) D1 and D6.)** Every
  ...
  is replaced for another reason — raising the namespace's `enforce` label to `restricted`, or
  ...
  **the node evidence is local, not CI** — the restricted-namespace e2e and the fleet-upgrade
  ...
  branch has not been through the pipeline, `TestE2E_FleetUpgrade` is not a CI job and started
